...
$replacements = @(
    # Use -creplace for Aset to avoid replacing 'dataset' or 'aset.framer.website'
    ...
    # Hero subtitle text spans (JS chunks strings)
    @{ Old = '"Our"'; New = '"Vritti"' }
    @{ Old = '"innovative"'; New = '"analyzes"' }
    @{ Old = '"AI"'; New = '"your"' }
    @{ Old = '"technology"'; New = '"trading"' }
    ...
    # ── 9. TESTIMONIALS → "WHY I BUILT THIS" ─────────────────────
    ...
    # ── 12. FOOTER ─────────────────────────────────────────────────
    @{ Old = '2026 Copyright © Aset\. All rights reserved\.'; New = '2026 Vritti — Built for a Zerodha AI Internship Application' }
    @{ Old = '2025 Copyright © Aset\. All rights reserved\.'; New = '2026 Vritti — Built for a Zerodha AI Internship Application' }

    # ── 13. LINKS & BRANDING (Only apply to HTML for paths, to avoid breaking JS bundles imports)
    # We will do these separately below
)
...
$htmlReplacements = @(
    ...
    @{ Old = 'href="https://www\.framer\.com\?via=green13"'; New = 'href="/analyse"' }
    @{ Old = 'href="https://easyframe\.framer\.website/"'; New = 'href="/about"' }
    ...
    @{ Old = 'href="\.\/#hero"'; New = 'href="#hero"' }
    @{ Old = 'href="\.\/#bento"'; New = 'href="#bento"' }
    @{ Old = 'href="\.\/#benefit"'; New = 'href="#benefit"' }
    @{ Old = 'href="\.\/#pricing"'; New = 'href="#pricing"' }
    @{ Old = 'href="\.\/#testimonials"'; New = 'href="#testimonials"' }
    @{ Old = 'href="\.\/#faq"'; New = 'href="#faq"' }

    @{ Old = '>Feature<'; New = '>Dashboard<' }
    @{ Old = '>Benefit<'; New = '>Story<' }
    @{ Old = '>Pricing <'; New = '>Analyse<' }
    @{ Old = '>Testimonials<'; New = '>About<' }

    @{ Old = 'href="#pricing">Analyse<'; New = 'href="/analyse">Analyse<' }
    @{ Old = 'href="#testimonials">About<'; New = 'href="/about">About<' }

    @{ Old = 'href="https://x\.com/"'; New = 'href="https://github.com"' }
    @{ Old = 'href="https://www\.instagram\.com/"'; New = 'href="/about"' }
...
foreach ($file in $filesToProcess) {
    if (Test-Path $file) {
        ...
        # Apply HTML specific replacements only to landing.html
        if ($file -match 'landing\.html') {
            foreach ($r in $htmlReplacements) {
                ...
                if ($file -match 'landing\.html') {
            $style = "<style>#pricing, #testimonials, #metrics, [data-framer-name='Social Trust'], [data-framer-name='Company logos'], [data-framer-name='Metrics Section'], [data-framer-name='Pricing Section'], [data-framer-name='Testimonials Section'] { display: none !important; }</style>"
...
Write-Host "Fixed React Hydration!"
